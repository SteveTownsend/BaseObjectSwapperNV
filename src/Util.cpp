...
namespace util
{
	std::vector<std::string> split_with_regex(const std::string& a_str,
		const srell::regex&                                      a_regex)
	{
		srell::sregex_token_iterator iter(a_str.begin(), a_str.end(), a_regex, -1);
  ...
  if (string::is_only_hex(a_str, true)) {
    ...
    if (const auto form = RE::TESForm::LookupByID(formID); !form) {
      logger::error(
          "\t\tFilter [{}] INFO - unable to find form, treating filter as cell "
          "formID",
          a_str);
    }
    return formID;
...
#if 0          
					logger::error("\t\t\tfailed to process {} (SWAP formID not found)",
						IDStr);
#endif
				}
			}
